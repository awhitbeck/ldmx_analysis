...
s = ldmx_container("/nfs/slac/g/ldmx/users/whitbeck/dos_electrones/ldmx-sw/ldmx_sim_events.root")
s.setup()
...
for i in xrange(33) :
    showerProfile_layer.append( r.TH1F("shower_profile_layer"+str(i),";r [mm];Hits*E_{dep} [MeV]",45,-98.149,98.149) )
    showerProfile_layer_rawPos.append( r.TH2F("shower_profile_rawPos_layer"+str(i),";r [mm];Hits*E_{dep} [MeV]",100,-40,40,100,-40,40) )
...
for ievt in xrange(totEvents):
    ...
    for h in s.ecalSimHits:
        ...
        ray_y_pos = recoil_ele_pos[0]+recoil_ele_mom[1]/recoil_ele_mom[2]*(pos[2]-recoil_ele_pos[2])
